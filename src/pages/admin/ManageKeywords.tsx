...
const ManageKeywords = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingItem) {
                await apiClient.put(`/keywords/${editingItem.id}/`, formData);
            } else {
                await apiClient.post('/keywords/', formData);
            }
            fetchData();
            handleCloseModal();
        } catch (error) {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold">Kalit So'zlarni Boshqarish</h1>
                <button onClick={() => handleOpenModal()} className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-sm">
                    <Plus size={20} className="mr-2" /> Yangi Kalit So'z
                </button>
            </div>
            <div className="bg-white shadow-md rounded-lg overflow-x-auto">
                <table className="min-w-full">
                    <thead className="bg-gray-100">
                        <tr>
                            <th className="text-left py-3 px-4 font-semibold text-sm">Nomi</th>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingItem ? "Kalit So'zni Tahrirlash" : "Yangi Kalit So'z Qo'shish"}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">Nomi</label>
